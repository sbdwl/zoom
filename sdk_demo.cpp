...
INT_PTR CALLBACK	StartMeetingDlg(HWND, UINT, WPARAM, LPARAM);
...
UINT64 g_userID = 0;
long long int g_number = 0;
wchar_t  g_userName[MAX_PATH] = { 0 };
...
//#error TODO:pass your domain
		param_.strWebDomain = _T("www.zoomus.cn");
...
//#error	TODO:pass your appKey and appSecret
		...
		authParam.appKey = L"wn4qqdHKCkizrAEndQaIZmhCCKn9EhbAavY0";
		authParam.appSecret = L"DMlXGwcIkcdpYFngmW4g7c9L4lBajbTjzBKp";
...
//#error	TODO:pass your user and meeting information 
					DialogBox(hInst, MAKEINTRESOURCE(IDD_STARTBOX), hWnd, StartMeetingDlg);
//ÐÞ¸Äpart
					ZOOM_SDK_NAMESPACE::StartParam startParam;
					startParam.userID = L"test_ucloud@looky.cc";
					startParam.userToken = L"123456";
					startParam.userName = g_userName;
					startParam.meetingNumber = g_number;
					startParam.userType = ZOOM_SDK_NAMESPACE::SDK_UT_APIUSER;
					startParam.isDirectShareDesktop = false;
					startParam.hDirectShareAppWnd = NULL;
		...
		case ID_SDK_STARTREC:
			{
					...
					&& IsInMeeting(g_pMeetService->GetMeetingStatus()))
				{
					...
					int sdk_error=g_pMeetService->StartRecording(startTime, L"c:\\dats1\\ddd");
...
INT_PTR CALLBACK StartMeetingDlg(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	UNREFERENCED_PARAMETER(lParam);
	switch (message)
	{
	...
	case WM_COMMAND:
		if (LOWORD(wParam) == IDOK || LOWORD(wParam) == IDCANCEL)
		{
			double g_numberlow = 0, g_numbermid = 0, g_numberhigh = 0;
			g_numberhigh = (UINT)GetDlgItemInt(hDlg, IDC_EDIT2, NULL, FALSE);
			g_numbermid = (UINT)GetDlgItemInt(hDlg, IDC_EDIT3, NULL, FALSE);
			g_numberlow = (UINT)GetDlgItemInt(hDlg, IDC_EDIT4, NULL, FALSE);
			g_number = g_numberlow + (g_numbermid * 10000);
			g_number = g_number + (g_numberhigh * 10000000);
			g_numberlow = 0; g_numbermid = 0, g_numberhigh = 0;

			GetDlgItemTextW(hDlg, IDC_EDIT1, g_userName, MAX_PATH);
			EndDialog(hDlg, LOWORD(wParam));
